...
void Relu_Pad_reflect_hw(TensorMem<T>& X) {
  int XN = X.shape.N, XH = X.shape.H - Size, XW = X.shape.W - Size,
      XC = X.shape.C;
  for (int n = 0; n < XN; n++)
    for (int c = 0; c < XC; c++) {
...
void Conv_CNorm_RPad_reflect(
    const Conv_Attributes& attributes, TensorMem<T>& X, TensorMem<T>& W,
    TensorMem<T>& B,
  ...
  if (x_shape.C % attributes.group || w_shape.N % attributes.group ||
      x_shape.C / attributes.group != w_shape.C ||
      attributes.kernel_shape[0] != w_shape.H ||
      attributes.kernel_shape[1] != w_shape.W || K_SIZE != w_shape.H ||
      K_SIZE != w_shape.W)
    assert(0 && "Error: Unappropriate input sizes for Conv !!\n");

  if (y_shape.H != (x_shape.H - (w_shape.H - 1) * attributes.dilations[0] +
                    attributes.pads[0] + attributes.pads[2] - 1) /
                           attributes.strides[0] +
                       1 ||
      y_shape.W != (x_shape.W - (w_shape.W - 1) * attributes.dilations[1] +
                    attributes.pads[1] + attributes.pads[3] - 1) /
...
#pragma HLS ARRAY_PARTITION variable = y_points complete
  ...
  for (int n = 0; n < batch; n++) {
    ...
    for (int g = 0; g < attributes.group; g++)
      for (int y_h = 0, e = 0; y_h < y_shape.H;
           y_h++, e += attributes.strides[0])
        for (int y_w = 0, f = 0; y_w < y_shape.W;
             y_w++, f += attributes.strides[1]) {
          for (int co = 0; co < C_out; co++) {
            y_shape.C = w_shape.N = b_shape.C = g * C_out + co;
            x_shape.H = start_height + e, x_shape.W = start_width + f;
            for (int i = 0; i < buff_size; i++) y_points[i] = 0;

            for (int w_h = 0, i = 0; w_h < K_SIZE;
                 w_h++, i += attributes.dilations[0])
              for (int w_w = 0, j = 0; w_w < K_SIZE;
                   w_w++, j += attributes.dilations[1]) {
                ...
                for (int r = 0; r < rolls; r++) {
                  if ((PEs & 7) || !INTRINMINH) {
                    for (int ci = 0; ci < PEs; ci++) {
                      ...
                      y_points[id] += X.get(x_shape.N, h, w, x_shape.C) *
                                      W.get(w_shape.N, w_h, w_w, w_shape.C);
                    }
                  } else {
                    ...
                    for (int ci = 0; ci < PEs; ci += 8) {
                      ...
                      __m128i vec_x16 = _mm_loadu_si128(
                          (__m128i*)X.raw_at(x_shape.N, h, w, x_shape.C));
                      __m128i vec_w16 = _mm_loadu_si128(
                          (__m128i*)W.raw_at(w_shape.N, w_h, w_w, w_shape.C));
                    ...
                    // Bước 2.1: Tách thanh ghi 256-bit làm 2 nửa 128-bit (mỗi
                    // nửa 4 số float) và cộng lại với nhau
                    __m128 lo_128 =
                        _mm256_castps256_ps128(sum_vec);  // Lấy 4 float thấp
                    __m128 hi_128 =
                        _mm256_extractf128_ps(sum_vec, 1);  // Lấy 4 float cao
                    __m128 sum_128 =
                        _mm_add_ps(lo_128, hi_128);  // Cộng lại -> Còn 4 float

                    // Bước 2.2: Dùng lệnh hadd (Horizontal Add) để gập 4 float
                    // thành 2 float, rồi thành 1 float
                    sum_128 = _mm_hadd_ps(sum_128,
                                          sum_128);  // Cộng các cặp cạnh nhau
                                                     // (còn 2 giá trị hữu ích)
                    sum_128 = _mm_hadd_ps(
                        sum_128, sum_128);  // Cộng lần nữa (còn 1 giá trị hữu
                                            // ích nằm ở vị trí đầu)

                    // Bước 2.3: Trích xuất số float duy nhất ra biến C++ thông
                    // thường
            ...
            Y.at(y_shape.N, y_h, y_w, y_shape.C) =
                y_points[0] + B.get(b_shape.N, b_shape.H, b_shape.W, b_shape.C);
          }
          ...
          // }
          // mean /= num;
          // var = (T) 1 / static_cast<T>(sqrtf(var - mean* mean*
          // adjustment_scale + epsilon)); for (int co = 0; co < C_out; co++) {
