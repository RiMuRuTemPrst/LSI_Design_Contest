...
#pragma HLS LOOP_TRIPCOUNT min=4 max=30 avg=14
    ...
    // Keeps this copy's balanced-tree reduction + var<0 clamp (diverged from upconv_core copy).
...
#pragma HLS DEPENDENCE variable=row_acc   type=inter false
        ...
        if (cs == C_OUT - 1) {
            float sum   = ((sum_rot[0]+sum_rot[1])+(sum_rot[2]+sum_rot[3]))+((sum_rot[4]+sum_rot[5])+(sum_rot[6]+sum_rot[7]));
            float sumsq = ((sumsq_rot[0]+sumsq_rot[1])+(sumsq_rot[2]+sumsq_rot[3]))+((sumsq_rot[4]+sumsq_rot[5])+(sumsq_rot[6]+sumsq_rot[7]));
