...
// NOTE: dst_stride = MODEL_C/16 (60), NOT ci_words. The engine reads X at the
// INTEGRATION layout (latent buffer 16x16x960 -> 60 words/pixel, CBI uses the
// first 14). Packing tightly at 14 words/px made the engine read OOB -> the
// SIGSEGV that has been failing test.cpp's CBI since the first fill csim.
static void pack_cbi_input(const float* src, data_256_t* dst, int HW, int C_IN, int CI_PAD, int dst_stride) {
...
int main() {
    ...
    vector<data_256_t> cbi_X_hls(HW * (MODEL_C/16)), cbi_W_hls(C_OUT * 9 * CI_WORDS);  // X at integration stride 60
    ...
    pack_cbi_input(cbi_x.data(),  cbi_X_hls.data(),   HW, C_CBI, CI_PAD, MODEL_C/16);
