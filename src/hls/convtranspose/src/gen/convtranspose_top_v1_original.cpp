...
#else
typedef data_t wide_t;  // C-sim: wide_t = data_t ? kh̫ng c?n pack
#endif

extern "C" {

void HW_ConvTranspose_0(wide_t* X, wide_t* W, wide_t* B, data_t* Y) {
...
void HW_ConvTranspose_1(wide_t* X, wide_t* W, wide_t* B, data_t* Y) {
...
void HW_ConvTranspose_2(wide_t* X, wide_t* W, wide_t* B, data_t* Y) {
    ...
    #pragma HLS INTERFACE m_axi port=Y offset=slave bundle=gmem3 depth=1966080 max_write_burst_length=256
    #pragma HLS INTERFACE s_axilite port=X      bundle=control
    #pragma HLS INTERFACE s_axilite port=W      bundle=control
    #pragma HLS INTERFACE s_axilite port=B      bundle=control
    #pragma HLS INTERFACE s_axilite port=Y      bundle=control
...
void HW_ConvTranspose_3(wide_t* X, wide_t* W, wide_t* B, data_t* Y) {
    ...
    #pragma HLS INTERFACE m_axi port=Y offset=slave bundle=gmem3 depth=3932160 max_write_burst_length=256
    #pragma HLS INTERFACE s_axilite port=X      bundle=control
    #pragma HLS INTERFACE s_axilite port=W      bundle=control
    #pragma HLS INTERFACE s_axilite port=B      bundle=control
    #pragma HLS INTERFACE s_axilite port=Y      bundle=control
