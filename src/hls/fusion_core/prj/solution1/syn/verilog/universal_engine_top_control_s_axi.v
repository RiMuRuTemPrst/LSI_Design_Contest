...
// ==============================================================
`timescale 1ns/1ps
module universal_engine_top_control_s_axi
#(parameter
    ...
    output wire [63:0]                   G,
    output wire [63:0]                   BE,
    output wire [63:0]                   G_IN,
    output wire [63:0]                   BE_IN,
    output wire [63:0]                   Y,
...
localparam
    ...
    ADDR_G_DATA_1       = 7'h38,
    ADDR_G_CTRL         = 7'h3c,
    ADDR_BE_DATA_0      = 7'h40,
    ADDR_BE_DATA_1      = 7'h44,
    ADDR_BE_CTRL        = 7'h48,
    ADDR_G_IN_DATA_0    = 7'h4c,
    ADDR_G_IN_DATA_1    = 7'h50,
    ADDR_G_IN_CTRL      = 7'h54,
    ADDR_BE_IN_DATA_0   = 7'h58,
    ADDR_BE_IN_DATA_1   = 7'h5c,
    ADDR_BE_IN_CTRL     = 7'h60,
    ADDR_Y_DATA_0       = 7'h64,
    ADDR_Y_DATA_1       = 7'h68,
    ADDR_Y_CTRL         = 7'h6c,
    ADDR_EPSILON_DATA_0 = 7'h70,
    ADDR_EPSILON_CTRL   = 7'h74,
    ADDR_MODE_DATA_0    = 7'h78,
    ADDR_MODE_CTRL      = 7'h7c,
    WRIDLE              = 2'd0,
    WRDATA              = 2'd1,
    WRRESP              = 2'd2,
    WRRESET             = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_G = 'b0;
    reg  [63:0]                   int_BE = 'b0;
    reg  [63:0]                   int_G_IN = 'b0;
    reg  [63:0]                   int_BE_IN = 'b0;
    reg  [63:0]                   int_Y = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_BE_DATA_0: begin
                    rdata <= int_BE[31:0];
                end
                ADDR_BE_DATA_1: begin
                    rdata <= int_BE[63:32];
                end
                ADDR_G_IN_DATA_0: begin
                    rdata <= int_G_IN[31:0];
                end
                ADDR_G_IN_DATA_1: begin
                    rdata <= int_G_IN[63:32];
                end
                ADDR_BE_IN_DATA_0: begin
                    rdata <= int_BE_IN[31:0];
                end
                ADDR_BE_IN_DATA_1: begin
                    rdata <= int_BE_IN[63:32];
                end
                ADDR_Y_DATA_0: begin
                    rdata <= int_Y[31:0];
                end
...
assign G             = int_G;
assign BE            = int_BE;
assign G_IN          = int_G_IN;
assign BE_IN         = int_BE_IN;
assign Y             = int_Y;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BE_DATA_0)
            int_BE[31:0] <= (WDATA[31:0] & wmask) | (int_BE[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BE_DATA_1)
            int_BE[63:32] <= (WDATA[31:0] & wmask) | (int_BE[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_G_IN_DATA_0)
            int_G_IN[31:0] <= (WDATA[31:0] & wmask) | (int_G_IN[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_G_IN_DATA_1)
            int_G_IN[63:32] <= (WDATA[31:0] & wmask) | (int_G_IN[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BE_IN_DATA_0)
            int_BE_IN[31:0] <= (WDATA[31:0] & wmask) | (int_BE_IN[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BE_IN_DATA_1)
            int_BE_IN[63:32] <= (WDATA[31:0] & wmask) | (int_BE_IN[63:32] & ~wmask);
    end
end
