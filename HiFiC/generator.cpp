...
void Hific_model(TensorMem<_Float16> &input, TensorMem<float> &output) {
    ...
    conv_block_init(input, flip, cbi_gamma_0, cbi_beta_0, e, 
    ...
    Conv_Attributes att(1, 1, 1, 7, 7, 0, 0, 0, 0, 1, 1);
...
int main() {
    ...
    float* output_data = ARENA_2.alloc<float>(BATCH_SIZE* 256* 256* 3);
    TensorMem<float> output(output_data, output_shape, false);
