...
void Hific_model(TensorMem<_Float16>& input, TensorMem<_Float16>& output) {
  ...
  conv_block_init(input, flip, cbi_gamma_0, cbi_beta_0, e, cbi_weight, cbi_bias,
                  cbi_gamma_3, cbi_beta_3, e);
  ...
  TensorMem<_Float16> upconv_block_1(upconv_block_1_data, upconv_block_1_shape,
                                     false);
  ...
  TensorMem<_Float16> upconv_block_2(upconv_block_2_data, upconv_block_2_shape,
                                     false);
  ...
  TensorMem<_Float16> upconv_block_3(upconv_block_3_data, upconv_block_3_shape,
                                     false);
  ...
  TensorMem<_Float16> upconv_block_4(upconv_block_4_data, upconv_block_4_shape,
                                     false);
  ...
  ARENA_1.pop();  // free(upconv_block_4_data)
}
...
int main() {
  ...
  read_tensor("io_params_2/Gen_cbi_cbi0_ReduceMean_1.txt", input);
  ...
  Hific_model(input, output);
  end = clock();

  write_tensor("io_params_2/generator_output.txt", output, 20);

  std::cout << "Arena_1 max size: " << ARENA_1.max_runtime_size << "\n"
            << "Arena_2 max size: " << ARENA_2.max_runtime_size << "\n"
            << "Total runtime: " << ((double)(end - start)) / CLOCKS_PER_SEC
            << " s\n";
